...
- find or retrieve conversation messages containing a unique marker
...
Codex should use the platform CLI prompt argument shape, for example:

```bash
codex [OPTIONS] "<unique prompt>"
```

Claude Code should use its equivalent non-interactive prompt path. If a platform
cannot produce an answer through the chosen invocation, that is a scenario
failure for v1 because the extended smoke target includes assistant-message
persistence.
...
1. JUnit verifies `NAMS_API_KEY` and `NAMS_WORKSPACE_ID` are present.
...
5. The test checks the platform CLI version inside the container.
...
8. The test installs project-local hook config:
   - Codex: project `.codex/hooks.json` plus any non-interactive local config
     needed to enable and trust hooks.
   - Claude Code: project `.claude/settings.local.json` plus any
     non-interactive local config needed to run hooks.
...
13. The test extracts the NAMS conversation ID or searches by the unique marker.
14. REST-assured verifies the user prompt was persisted.
15. REST-assured verifies an assistant message for the run was persisted.
...
- no NAMS conversation or message match for the unique marker
...
- bearer tokens
- raw API keys
- any environment dump containing `NAMS_API_KEY`
...
`NAMS_BASE_URL` is optional. Required inputs are intentionally not skipped. A
developer who runs the live suite without secrets should get an immediate
failure explaining which inputs are missing.
