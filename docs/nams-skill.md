...
description: |
  Persistent memory system backed by Neo4j for maintaining context across conversations.
  Automatically stores and retrieves entities, messages, and reasoning traces.
  Supports semantic search of past interactions and entity relationship mapping.
---

# Neo4j Agent Memory

You have access to a persistent memory system backed by Neo4j. Use it automatically and silently — the user should never need to ask you to remember or recall things. Treat it as a natural extension of your own memory, not a tool you wait to be told to use.
...
After each user message and after each of your responses, store the turn:

```
memory_add_message(conversation_id=<conversation_id>, role="user", content=<message>)
memory_add_message(conversation_id=<conversation_id>, role="assistant", content=<your response>)
```
...
| `memory_add_message` | Store a message (triggers entity extraction) |
