...
<session-id> --workspace <workspace-id-or-name>`. When the hook exposes the
current session ID, the notice should render that concrete value and leave only
`<workspace-id-or-name>` for the user to replace.
