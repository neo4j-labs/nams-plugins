...
**Architecture:** Keep the walking skeleton narrow. `src/cli.ts` parses `nams-hooks run <harness>`, reads JSON from stdin, writes JSONL diagnostics into `.nams/logs/`, and emits hook-safe JSON on stdout. Templates point each platform's session-start hook at the compiled CLI.
