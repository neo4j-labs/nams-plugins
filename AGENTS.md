...
- `apiKey` and `baseUrl` are required for NAMS requests. `workspaceId` is required for memory requests unless the harness path supports workspace auto-resolution before memory starts.
