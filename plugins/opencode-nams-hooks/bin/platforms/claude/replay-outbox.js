...
export function claudeReplayOutboxRecords(sessions) {
    const records = [];
    for (const session of sessions) {
        ...
        const timeline = [
            ...session.messages.map((value) => ({ kind: "message", value })),
            ...session.steps.map((value) => ({ kind: "step", value })),
...
export async function createClaudeReplayOutbox(input) {
    ...
    let directory;
    try {
        directory = await mkdtemp(path.join(input.temporaryRoot ?? tmpdir(), "nams-hooks-claude-replay-"));
...
export async function readClaudeReplayOutbox(outboxPath) {
    let contents;
    try {
    ...
    for (const [index, line] of contents.split(/\r?\n/).entries()) {
        ...
        try {
            ...
            if (!isClaudeReplayOutboxRecord(parsed))
...
export function validateClaudeReplayOutboxReferences(records) {
...
export async function removeClaudeReplayOutbox(outbox) {
...
function isClaudeReplayOutboxRecord(value) {
