...
import { NamsClient, NamsWorkspaceClient, } from "../../generated/nams-client.js";
import { loadNamsConnectionConfig, } from "../../runtime/config.js";
import { serializeToolInput, serializeToolOutput, } from "../../runtime/memory-service.js";
...
import { readClaudeReplayOutbox, validateClaudeReplayOutboxReferences, } from "./replay-outbox.js";
export async function sendClaudeReplayOutbox(input) {
    ...
    for (const record of records) {
        if (record.kind === "conversation.create") {
            ...
            conversationIds.set(record.localConversationId, requiredId(response.id, "NAMS conversation response did not include id"));
        ...
        if (record.kind === "reasoningStep.create") {
            ...
            stepIds.set(record.localStepId, requiredId(response.id, "NAMS reasoning response did not include id"));
...
async function resolveDestination(input, onRequest) {
    let connection;
    try {
        connection = await loadNamsConnectionConfig(path.resolve(input.importRoot), discoverClaudeNamsConfig);
    }
    catch {
...
async function namsRequest(operation) {
    try {
